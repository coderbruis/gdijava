...
具体内容不在这里进行介绍。为方便读者能够理解下面的内容，请认真[总结Java内存区域和常量池](https://blog.csdn.net/CoderBruis/article/details/85240273)
...
1. 使用字面量的方式创建字符串
使用字面量的方式创建字符串，要分两种情况。① 如果字符串常量池中没有值，则直接创建字符串，并将值存入字符串常量池中；
...
```
        /**
         * 这里为true。因为"AABB"在常量池中不存在，所以代码String a7 = new String("AA") + new String("BB");
         * 经历的过程为如下：
         * 1. new String("AA")，先在堆中创建String对象，然后检查常量池中有没有"AA"的字面量，没有则在常量池中创建"AA"字面量；同理new String("BB")
         * 2. 然后new String("AA") + new String("BB")就相当于，"AA" + "BB"，底层就是相当于new StringBuilder().append("AA").append("BB").toString()，先判断
         *      字符串常量池中是否有"AABB"的字面量，没有则在字符串常量池中创建"AABB"的字面量，然后返回引用；
         *      如果有：
         *          判断这个常量是（new出来的）引用还是常量
         *          如果是引用，则返回引用；
         *          如果是常量，则返回常量；
         *
         *  所以a7.intern()是返回的引用。
        ...
        /**
         * false
         * 因为在常量池中已经存在了"ABABCDCD"的字面量，a9是指向常量池中的一个引用。
         * 而a8是指向堆中的一个引用。
         */
